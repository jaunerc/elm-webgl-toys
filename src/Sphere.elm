...
scene angle =
    [ WebGL.entity vertexShader fragmentShader (sphereMesh Color.green) (uniforms 0 0 angle)
    ]
...
sphereMesh : Color -> WebGL.Mesh Vertex
sphereMesh color =
    let
        rgba = Color.toRgba color

        triangles = splitOctahedronTriangles 4 octahedronTriangles

        verticesFromVec3 (a, b, c) =
            ( Vertex (vec3 rgba.red rgba.green rgba.blue) a
            , Vertex (vec3 rgba.red rgba.green rgba.blue) b
            , Vertex (vec3 rgba.red rgba.green rgba.blue) c)
    in
    WebGL.triangles (List.map verticesFromVec3 triangles)
